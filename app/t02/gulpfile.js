...
gulp.task('default',
	...
	gulp.series(
		...
		,function watcher(c){ //## 把这个写入server 也不错???!!!
			...
			console.log(
                mergeLink(config.root.src,config.scripts.listen),
                mergeLink(config.root.src,config.style.listen),
                mergeLink(config.root.src,config.html.listen),
                mergeLink(config.root.src,config.images.listen),
                mergeLink(config.root.src,config.sp.listen),
                mergeLink(config.root.src,config.fonts.listen),
                mergeLink(config.root.src,config.media.listen),
                mergeLink(config.root.src,config.swf.listen)
            );
